...
pub type ControllerEndpoint<C> = (Option<&'static str>, Method, &'static str, Box<dyn DynControllerHandler<C, Body> + Send + Sync>, Box<dyn GuardChain>);
...
impl<C: Controller> EndpointsBuilder<C> {
    ...
    {
        self.handlers.push((None, method, route, Box::new(handler), guards(GuardBuilder::default()).build()));
        self
    }
    ...
    pub fn add_with_name<H>(mut self, handler_name: &'static str, method: Method, route: &'static str, handler: H) -> Self
        where
            H: 'static + DynControllerHandler<C, Body> + Send + Sync,
    {
        self.handlers.push((Some(handler_name), method, route, Box::new(handler), GuardBuilder::default().build()));
        self
    }
    ...
    pub fn add_with_guards_and_name<H, F, Chain>(mut self, handler_name: &'static str, method: Method, route: &'static str, handler: H, guards: F) -> Self
        where
            ...
            Chain: GuardChain + 'static,
    {
        self.handlers.push((Some(handler_name), method, route, Box::new(handler), guards(GuardBuilder::default()).build()));
        self
    }

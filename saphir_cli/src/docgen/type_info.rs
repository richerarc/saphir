...
use crate::docgen::rust_module::RustModule;
            ...
            Type::Path(p) => {
                if let Some(s) = p.path.segments.first() {
                    ...
                    if name == "Vec" || name == "Option" {
                        ...
                        let type_path = file.find_impl(name.as_str()).ok().flatten().map(|i| i.file.use_path.clone());
                        return Some(TypeInfo { name, type_path, is_array: false, is_optional: false, min_array_len: None, max_array_len: None });
